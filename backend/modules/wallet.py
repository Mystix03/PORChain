...
async def send(to_address: str, amount: float) -> dict:
    ...
    if registry.phase_index(current_phase) < 3:
        raise ValueError(f"Security Gate: Your current phase ({current_phase}) is too low. SEND requires Phase 3 (Validator) or higher.")
...
async def stake(amount: float, reason: str = "VOUCH") -> dict:
    ...
    current_phase = await registry.get_phase(my_id)
    if registry.phase_index(current_phase) < 3:
        raise ValueError(f"Security Gate: Your current phase ({current_phase}) is too low. STAKE requires Phase 3 (Validator) or higher.")
    if current_phase == "BANNED":
...
async def unstake(amount: float, address: str = None, reason: str = "RELEASED") -> dict:
    ...
    my_id = identity.get()["node_id"]
    current_phase = await registry.get_phase(my_id)
    is_protocol_return = reason.startswith("GRADUATED") or reason.startswith("OBSERVATION_SHELTER")
    
    if current_phase == "BANNED" and not is_protocol_return:
        raise ValueError("BANNED nodes cannot manually unstake tokens")
        
    if registry.phase_index(current_phase) < 3 and not is_protocol_return:
        raise ValueError(f"Security Gate: Your current phase ({current_phase}) is too low. UNSTAKE requires Phase 3 or higher.")
